...
@Component({
  selector: "app-manager-salesdetails",
  templateUrl: "./manager-salesdetails.component.html",
  styleUrls: ["./manager-salesdetails.component.scss"],
})
export class ManagerSalesdetailsComponent implements OnInit {
  sale: any;
  total: any;
  grandTotal: any;
  address: any;
  ...
  ngOnInit() {
    this.title.setTitle(
      "Mini Mart | The biggest super market in Little India fro household items",
    );
    this.cols = [
    ...
    if (this.managerSalesService.sale === undefined) {
      this.location.back();
    } else {
      this.sale = this.managerSalesService.sale;
  ...
  getAddress(addressId) {
    this.managerSalesService.getAddress(addressId).subscribe((details) => {
  ...
  printPDFSale() {
    ...
    this.saleDetails.forEach((element) => {
      const total = element.item_price * element.quantity - element.discount;
      const row = [
        element.name,
        "SKU: " +
  ...
  getShopItems(branchId) {
    this.managerSalesService.getAllShopItems(branchId).subscribe((products) => {
  ...
  addNewSaleItem(form: NgForm) {
    ...
    // console.log(value);
    this.spinner.show();
    this.managerSalesService.addSaleItem(this.sale.sales_id, value).subscribe(
      (response) => {
        this.spinner.hide();
