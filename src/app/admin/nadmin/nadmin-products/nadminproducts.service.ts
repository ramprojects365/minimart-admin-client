...
@Injectable()
export class NadminProductsService {
    ...
    //             , item_price: product.price, item_discount: product.discount
    //             , articleNumber: product.articleNumber, remarks: product.remarks 
    //         });
    // }
    addShopItem(product, branchesToAdd) {
        return this.http.post<AdminProductsResponse>('api/v2/admin/web/shopitems',
            {
                branch_ids: branchesToAdd, product_id: product.product
                , item_price: product.price, item_discount: 0, max_quantity: product.max_items_per_order
                , articleNumber: product.articleNumber, remarks: '' 
            });
    }
        ...
        return this.http.patch<AdminProductsResponse>('api/v2/admin/web/shopitems/' + product.item_id,
            {
                ...
                max_quantity: product.max_items_per_order, item_qr_code: product.item_qr_code, item_quantity: product.item_quantity, articleNumber: product.articleNumber, remarks: product.remarks 
            });
    }
